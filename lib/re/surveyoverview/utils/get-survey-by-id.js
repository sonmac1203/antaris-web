...
export async function getSurveyById(props) {
  ...
  const result = await mdhClient.getSurveys({ accessToken, params });
  ...
  try {
    ...
    const promises = groupedSurveys.map(async (survey) => {
      const existingSurvey = await Survey.findOne(
        ...
        path: 'assigned_to.participant',
        select: 'participant_identifier secondary_identifier demographics',
      });
      if (existingSurvey) {
